...
def run_query(query, params=(), fetch="none"):
    """
    Thread-safe helper to run queries.
    fetch: "none", "one", "all"
    """
    with db_lock:
        try:
...
def get_active_item():
    return run_query("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team, unsold_timestamp FROM items WHERE is_active = 1 LIMIT 1", fetch="one")

def get_highest_bid(item_id):
    return run_query("SELECT team_name, amount FROM bids WHERE item_id = ? ORDER BY amount DESC LIMIT 1", (item_id,), fetch="one")
...
def place_bid(item_id, team_name, current_amount):
    # Use a lock to ensure atomic transaction
    with db_lock:
        try:
            cursor = conn.cursor()
            
            # Check if the item is already sold
            cursor.execute("SELECT winner_team, current_bid FROM items WHERE id = ?", (item_id,))
            item_details = cursor.fetchone()
            
            # Get the team's remaining budget
            cursor.execute("SELECT budget_remaining FROM teams WHERE name = ?", (team_name,))
            budget_res = cursor.fetchone()
            remaining_budget = budget_res[0] if budget_res else 0
            ...
            if item_details and item_details[0] and item_details[0] != 'UNSOLD':
                previous_winner = item_details[0]
                previous_amount = item_details[1]
                
                # Refund the previous team
                cursor.execute("UPDATE teams SET budget_remaining = budget_remaining + ? WHERE name = ?", (previous_amount, previous_winner))
                
                # Remove the item from sold_items table
                cursor.execute("DELETE FROM sold_items WHERE item_name = (SELECT name FROM items WHERE id = ?)", (item_id,))

            # Deduct budget from new bidder
            cursor.execute("UPDATE teams SET budget_remaining = budget_remaining - ? WHERE name = ?", (new_amount, team_name))

            cursor.execute("INSERT INTO bids (item_id, team_name, amount, timestamp) VALUES (?, ?, ?, ?)",
                      (item_id, team_name, new_amount, datetime.now().isoformat()))
            cursor.execute("UPDATE items SET current_bid = ?, winner_team = ? WHERE id = ?", (new_amount, team_name, item_id))
            ...
            conn.rollback()
            print(f"Database error in place_bid: {e}")
            if cursor:
                cursor.close()

def get_team_budget(team_name):
    result = run_query("SELECT budget_remaining FROM teams WHERE name = ?", (team_name,), fetch="one")
    return result[0] if result else 0

def update_team_budget(team_name, spent_amount):
    run_query("UPDATE teams SET budget_remaining = budget_remaining - ? WHERE name = ?", (spent_amount, team_name))

def get_all_items():
    return run_query("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team FROM items", fetch="all")
...
def set_active_item(item_id):
    with db_lock:
        try:
            ...
            cursor.execute("UPDATE items SET is_active = 1, winner_team = NULL, current_bid = base_price WHERE id = ?", (item_id,))
            conn.commit()
            cursor.close()
        except sqlite3.Error as e:
            conn.rollback()
            print(f"Error setting active item: {e}")

def stop_all_bidding():
    # This function needs a transaction
    with db_lock:
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team, unsold_timestamp FROM items WHERE is_active = 1 LIMIT 1")
            active = cursor.fetchone()
            
            if active:
                item_id = active[0]
                cursor.execute("SELECT team_name, amount FROM bids WHERE item_id = ? ORDER BY amount DESC LIMIT 1", (item_id,))
                highest = cursor.fetchone()
                
                if highest:
                    winner, amount = highest
                    # Update budget
                    cursor.execute("UPDATE teams SET budget_remaining = budget_remaining - ? WHERE name = ?", (amount, winner))
                    cursor.execute("UPDATE items SET winner_team = ? WHERE id = ?", (winner, item_id))
                    
                    # Insert sold item into sold_items table
                    cursor.execute("INSERT INTO sold_items (item_name, sold_amount, rating, category, nationality, team_bought, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
                              (active[1], amount, active[2], active[3], active[4], winner, datetime.now().isoformat()))
                    
                    # Remove from unsold_items table
                    cursor.execute("DELETE FROM unsold_items WHERE item_name = ?", (active[1],))
            ...
            conn.rollback()
            print(f"Error stopping bidding: {e}")
    return None

def get_team_budgets():
    return run_query("SELECT name, budget_remaining, logo_url FROM teams", fetch="all")
...
def mark_as_unsold(item_id):
    with db_lock:
        try:
            ...
            cursor.execute("UPDATE items SET winner_team = 'UNSOLD', is_active = 0, unsold_timestamp = ? WHERE id = ?", 
                     (timestamp, item_id))
            
            # Get item details to insert into unsold_items table
            cursor.execute("SELECT name, rating, category, nationality FROM items WHERE id = ?", (item_id,))
            item_details = cursor.fetchone()
            
            if item_details:
                cursor.execute("INSERT INTO unsold_items (item_name, rating, category, nationality, status, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
            ...
            conn.rollback()
            print(f"Error marking unsold: {e}")
            return None
    ...
    with db_lock:
        try:
            ...
            if item_name:
                ...
                cursor.execute("DELETE FROM bids WHERE item_id = ?", (item_id,))
                # Delete from sold_items and unsold_items tables
                cursor.execute("DELETE FROM sold_items WHERE item_name = ?", (item_name,))
                cursor.execute("DELETE FROM unsold_items WHERE item_name = ?", (item_name,))
...
def get_team_squad_info(team_name):
    ...
    players = run_query("SELECT name, rating, category, nationality FROM items WHERE winner_team = ?", (team_name,), fetch="all")
    ...
    for player in players:
        ...
        sold_amount_result = run_query("SELECT sold_amount FROM sold_items WHERE item_name = ?", (player_name,), fetch="one")
    ...
    budget_result = run_query("SELECT budget_remaining FROM teams WHERE name = ?", (team_name,), fetch="one")
...
def get_sold_amount(item_name):
    result = run_query("SELECT sold_amount FROM sold_items WHERE item_name = ?", (item_name,), fetch="one")
    return result[0] if result else 0
...
def reset_all_data():
    ...
    with db_lock:
        try:
            ...
            # Clear unsold_items table
            cursor.execute("DELETE FROM unsold_items")
            
            # Reset team budgets to initial budgets
            cursor.execute("UPDATE teams SET budget_remaining = initial_budget")
...
def export_all_data():
    ...
    try:
        # Get all items data
        items_data = run_query("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team, unsold_timestamp FROM items", fetch="all")
        
        # Get all teams data
        teams_data = run_query("SELECT name, budget_remaining, logo_url, initial_budget FROM teams", fetch="all")
        
        # Get all bids data
        bids_data = run_query("SELECT id, item_id, team_name, amount, timestamp FROM bids", fetch="all")
        
        # Get all sold items data
        sold_items_data = run_query("SELECT id, item_name, sold_amount, rating, category, nationality, team_bought, timestamp FROM sold_items", fetch="all")
        
        # Get all unsold items data
        unsold_items_data = run_query("SELECT id, item_name, rating, category, nationality, status, timestamp FROM unsold_items", fetch="all")
...
if 'admin_authenticated' in st.session_state and st.session_state['admin_authenticated']:
    admin_tab = st.sidebar.radio("Admin Functions", ["Manage Teams", "Manage Players", "Manage Sponsors", "Activate Bidding", "Download Data", "Reset Data"])
    ...
    if admin_tab == "Manage Teams":
        ...
        if st.sidebar.button("Add Team") and new_team_name and team_password:
            run_query("INSERT OR REPLACE INTO teams (name, budget_remaining, logo_url, initial_budget, password) VALUES (?, ?, ?, ?, ?)",
        ...
        teams = run_query("SELECT name, budget_remaining, logo_url, initial_budget FROM teams", fetch="all")
        ...
        for team in teams:
            with st.sidebar.expander(f"Team: {team[0]}"):
                ...
                if st.button(f"Update {team[0]}", key=f"update_{team[0]}"):
                    ...
                    run_query("UPDATE teams SET budget_remaining = ?, logo_url = ? WHERE name = ?", 
                ...
                if st.button(f"Delete {team[0]}", key=f"del_{team[0]}"):
                    run_query("DELETE FROM teams WHERE name = ?", (team[0],))
                    st.rerun()
        ...
        item_category = st.sidebar.selectbox("Player Specialization", ["Batsman", "Bowler", "Allrounder", "Wicketkeeper"])
        item_nationality = st.sidebar.selectbox("Player Nationality", ["India", "Afghanistan", "Australia", "Bangladesh", "England","New Zealand", "Sri Lanka", "South Africa","West Indies", "Other"])
        ...
        if st.sidebar.button("Add Item") and item_name:
            try:
                ...
                run_query("INSERT INTO items (name, rating, category, nationality, image_url, base_price) VALUES (?, ?, ?, ?, ?, ?)",
        ...
        if st.sidebar.button("Add Sponsor") and new_sponsor_name and new_sponsor_logo:
            run_query("INSERT INTO sponsors (name, logo_url) VALUES (?, ?)", (new_sponsor_name, new_sponsor_logo))
            st.sidebar.success(f"Sponsor '{new_sponsor_name}' added.")
            
        # List and delete sponsors
        st.sidebar.markdown("### Existing Sponsors")
        sponsors = run_query("SELECT id, name, logo_url FROM sponsors", fetch="all")
            ...
            with st.sidebar.expander(f"{sponsor[1]}"):
                ...
                with col1:
                    if st.button("Update", key=f"upd_{sponsor[0]}"):
                        run_query("UPDATE sponsors SET name = ?, logo_url = ? WHERE id = ?", (new_name, new_logo, sponsor[0]))
                        st.success("Updated!")
                        st.rerun()
                with col2:
                    if st.button("Delete", key=f"del_{sponsor[0]}"):
                        run_query("DELETE FROM sponsors WHERE id = ?", (sponsor[0],))
                        st.rerun()
        ...
        if selected_item_name:
            ...
            if st.sidebar.button("Stop Current Bidding"):
                sale_details = stop_all_bidding()
                
                if sale_details:
                    winner, amount, item_name = sale_details
                    st.sidebar.success("Bidding stopped and winner updated.")
                    st.rerun()

        # Sponsor Logo
        title_sponsor = run_query("SELECT logo_url FROM sponsors WHERE name = 'Title Sponsor'", fetch="one")
        if title_sponsor:
        ...
        try:
            items_count = run_query("SELECT COUNT(*) FROM items", fetch="one")[0]
            teams_count = run_query("SELECT COUNT(*) FROM teams", fetch="one")[0]
            bids_count = run_query("SELECT COUNT(*) FROM bids", fetch="one")[0]
            sold_count = run_query("SELECT COUNT(*) FROM sold_items", fetch="one")[0]
            unsold_count = run_query("SELECT COUNT(*) FROM unsold_items", fetch="one")[0]
...
with tab1:
    ...
    if active_item:
        item_id, item_name, item_rating, item_category, item_nationality, item_image_url, item_base_price, current_bid, is_active, winner, unsold_timestamp = active_item
...
''', unsafe_allow_html=True)
        ...
        if recent_unsold:
            ...
            if last_sold:
                ...
                try:
                    ...
                    if current_ts - ts < 4:
                        c.execute("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team, unsold_timestamp FROM items WHERE name = ?", (name,))
    ...
    if 'admin_authenticated' not in st.session_state or not st.session_state['admin_authenticated']:
        ...
        with c1:
            # Show Select Team
            selected_team = st.selectbox("Select Team", team_names, label_visibility="collapsed", index=None, placeholder="Select Team")
        ...
        if selected_team_details:
            ...
            if len(selected_team_details) == 3:
                ...
                # Only show bid button if password is verified AND active item exists AND no animation is playing
                with c3:
                    if password_verified and active_item and not recent_status:
    ...
    sponsor_html = '<div style="display: flex; flex-wrap: wrap; gap: 7px; justify-content: center; margin-top: 7px; background: #ffffff; padding: 15px; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">'
    for sponsor in sponsors:
        sponsor_html += f"""<div style="background: #ffffff; border-radius: 12px; padding: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); width: 100px; height: 80px; display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; border: 1px solid rgba(0,0,0,0.05);" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 5px 15px rgba(0,0,0,0.1)'" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 5px rgba(0,0,0,0.05)'"><img src="{sponsor['logo']}" alt="{sponsor['name']}" style="max-width: 100%; max-height: 100%; object-fit: contain;"></div>"""
    sponsor_html += '</div>'
...
with tab3:
    ...
    if selected_team_name:
        ...
        if players:
            players_df = pd.DataFrame(players, columns=["Player Name", "Rating", "Specialization", "Nationality"])
