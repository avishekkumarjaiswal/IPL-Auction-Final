...
def get_active_item():
    c.execute("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team, unsold_timestamp FROM items WHERE is_active = 1 LIMIT 1")
    return c.fetchone()
...
def place_bid(item_id, team_name, current_amount):
    ...
    c.execute("UPDATE items SET current_bid = ? WHERE id = ?", (new_amount, item_id))
    conn.commit()
...
def get_all_items():
    c.execute("SELECT id, name, rating, category, nationality, image_url, base_price, current_bid, is_active, winner_team FROM items")
    return c.fetchall()

def set_active_item(item_id):
    c.execute("UPDATE items SET is_active = 0")
    c.execute("UPDATE items SET is_active = 1, winner_team = NULL, current_bid = base_price WHERE id = ?", (item_id,))
    conn.commit()
